function render_template(uri, tplName, renderTo, data){
	if (!(tplName in ich.templates)){
		$.get(uri, function (template){
				ich.addTemplate(tplName, template);
				$(renderTo).html(ich[tplName](data));
				$("body").trigger( "templateChange", [tplName]);
			});
		}
	else{
		$(renderTo).html(ich[tplName](data));
